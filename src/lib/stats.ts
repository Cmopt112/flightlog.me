...
const realFlights = (flights: Flight[]) => flights.filter((f) => !f.isSimulator)

export function totalHours(flights: Flight[]): number {
  return round2(sum(realFlights(flights).map((f) => f.totalTimeDecimal)))
}
...
export function hoursSince(flights: Flight[], since: Date): number {
  return round2(
    sum(
      realFlights(flights)
        .filter((f) => new Date(f.date) >= since)
        .map((f) => f.totalTimeDecimal),
    ),
  )
}
